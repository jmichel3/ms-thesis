...
Interestingly, both our inharmonicity estimation reliability and performance dynamic level don't explain the large variance we're seeing in inharmonicity estimates, even though no other variables aside from finger vs plectrum plucking are at play in the measurements in Figure~\ref:{fig:best-worst-r2}. Plucking with the finger vs. with a plectrum wasn't investigated in this work, so it remains unclear why such large variance persists in measurements on a single guitar. 

However, many more factors could be contributing to the inter-guitar inharmonicity measurement variance. Differences in the physical properties of the various RWC guitars' strings (like gauge, wound vs. unwound, length due to intonation adjustments, stiffness due to age) could be vast, and each of these factors are indeed components in the definition of inharmonicity in equation~\eqref{eq:beta}. Details regarding these instrument-specific factors aren't recorded in the RWC annotations, so it's plausible that our system's electric gutiar transcription difficulty is exacerbated by unknown combinations of these confounding factors. To get an idea of the range of inharmonicity changes imparted by different values of these factors, we performed the following experiment. We recorded 0.09, 0.10, and 0.11 set gauges of the unwound third string of a personal electric guitar (Ibanez RG), as well as another take of a 0.11 set with a wound third string. We also had two sets of 0.10 set gauges, one of which was old and the other which was new, in an attempt to examine the effect of its possible age-related stiffness. Finally, we recorded each of the previous settings at two intonation settings: the string's nominal ----mm length and a 5mm extension. Statistics of each se	ttings' inharmonicity measurements are shown in Figure~\ref{fig:physical-factors}.
