...
\hline
\end{tabular}
\caption{Error probability comparisons. Results from our baseline (``Bayes") and regreesion (``Regr.") methods which differ from PCT to a statistically-significant ($\alpha=0.05$) degree are shaded in gray. Electric guitar trials and ``Guitar-independent" scenarios are omitted since we're comparisons in those cases aren't available.}
\label{tab:ttest-RWC}
\end{center}
\end{table}
...
\section{Future Work}
Inharmonicity-based tablature is a fascinating topic, and future researchers should advance some of the findings in this thesis. In the previous section, we confirmed that inter-guitar geometry parameters played a significant role in inter-guitar inharmonicity measurement variability. This makes clear that understanding and accounting for the factors that change a system's inharmonicity measurement is key for good performance. More work should be done on characterizing the expected perturbations in inharmonicity estimation given a particular change in string gauge, length, winding, or stiffness. There's also much we didn't solve about our intra-guitar inharmonicity variability -- future work should seek to answer why inharmonicity variability remains high on just one RWC guitar (i.e., eliminating inter-guitar variability) in some cases.

Though the inharmonicity literature is quite established, new researchers would do well to acquaint themselves with both the physics of the phenomenon and the history of algorithms developed for its estimation. It's naturally the most critical feature in an inharmonicity-based transcription system, and understanding its past will aid in the achievement of robust estimation and, potentially, innovations to the estimation task. Because of its importance, much more work is needed in iLastly, 

New researchers should also consider ways 
...
A cursory investigation of the cause for the poor electric guitar transcription performance revealed that inharmonicity estimation for the instrument is less reliable, particularly on string 3, and the electric guitars' pickups aren't to blame. It was found that factors related to the guitar geometry (string gauge, length, stiffness, winding) certainly induce changes in inharmonicity, which helps explain high inter-guitar inharmonicity variance, though it remains unclear why intra-guitar inharmonicity variance was still high for some of the RWC guitars' strings. We closed with limitations and future scope, cautioning against misinformed application of our reported results to real-world guitar scenarios, and highlighting musical context exploitation, experiments with other classifiers, and different regression polynomials as potential areas for elaboration.
