...
The first method linearly regresses guitar strings' log-inharmonicities against their pitches to obtain characteristic trajectories, then assigns unseen notes to the strings whose trajectories' means and variances maximize the probability of the notes' measured inharmonicities. The second method, developed as a baseline against which to compare the first and which we refer to as simple Bayesian classification, characterizes the inharmonicity distribution of each fretboard position as a normal probability density, then similarly assigns unseen notes to the fretboard positions that maximize the likelihood of their observed inharmonicities. Results from classical and acoustic guitars in the standard Real World Corpus of guitar recordings show that our regression method generally performs better than the existing inharmonicity-based system for test recordings on which the system wasn't individually trained, as well as show that leveraging the linear structure of log-inharmonicity trajectories increases overall performance compared to the baseline method. Also, results from personally-recorded acoustic and electric guitars in various alternate tunings show that our tuning compensation is successful at predicting tuning-related inharmonicity changes and improving transcription performance.
...
We performed paired-sample $t$-tests to assess whether differences in transcription error between the existing PCT method and each of our novel methods were statistically significant at the $\alpha = 0.05$ level. We differenced the PCT method's and comparison method's (either Bayes or Regression) error probabilities for each row in Table~\ref{tab:ttest-RWC} to obtain $\mu_d$, the mean of the distribution of the differences. Because we didn't have access to PCT's trial-by-trial transcription results, we approximated the standard deviation $\sigma_d$ of the distribution of the differences by the square root of the average of the methods' Bernoulli approximation of variances, $\sigma_d = \sqrt{(\sigma^2_1+\sigma^2_2)/2}$, where $\sigma^2 = p(1-p)$ and the success parameter $p$ is simply taken to be $p = 1-\mu_d$. We calculated each comparison's $t$-score according to $t = \mu_d/(\sigma_d/\sqrt{n})$, with $n=936$ for the number of classified instances over which the error probabilities were averaged (and $n=2808$ for the overall error measures), then summed the right and left tails of the c.d.f of the student's $t$-distribution at $\pm t$. This gives us the probability, or $p$-value, of observing the classifiers' performance differences due to chance given that the null hypothesis of zero-mean performance differences is true. In Table~\ref{tab:ttest-RWC}, we highlight transcription results from the $t$-tests which yielded $p$-values less than $\alpha = 0.05$, allowing us to reject the null hypothesis in favor of the alternative that the classifiers' performance differences don't have zero-mean, i.e. that the highlighted results differ significantly from PCT. The best-performing algorithm for individual guitars alternates between the three methods depending on the guitar and evaluation scenario, but the ``Total Overall" row shows that our regression method improves upon PCT's performance under the ``Guitar-averaged" scenario to a statistically significant degree across all classicals and acoustics.
...
\hline
\hline
Total Overall: &   \cellcolor[gray]{0.8}\bf{0.05} & \bf{0.01} &   \cellcolor[gray]{0.8}\bf{0.04} &  \cellcolor[gray]{0.8}\bf{0.09} & \bf{0.07} &  \cellcolor[gray]{0.8}\bf{0.05} & \bf{--} & -- & \bf{--}\\
\hline
%Overall Average: & \bf{0.08} & \bf{0.01} & \bf{0.06} & \bf{0.12} & \bf{0.13} & \bf{0.09} & \bf{0.14} & -- & \bf{0.12}\\
\hline
\end{tabular}
...
\hline
\hline
Total Overall: & \bf{0.08} & \bf{--} & \cellcolor[gray]{0.8}\bf{0.07} & \bf{0.12} & \bf{--} & \cellcolor[gray]{0.8}\bf{0.09} & \bf{0.14} & -- & \cellcolor[gray]{0.8}\bf{0.12}\\
\hline
\end{tabular}
...
\end{equation}
showing us that, for a maximum string gauge increase of $a=1.67$, we can expect a variation in log-inharmonicity of approximately $4\log_21.67 \approx 3$ units.
...
\end{equation}
In words, for a scaling factor $b$ on string length, we can expect our log-inharmonicity measurements to vary by $-4\log_2b$. For a 10mm intonation extension on a string which measures a conservative 600mm long, $b$ amounts to roughly $1.0167$ yielding inharmonicity perturbations of $-4\log_2b = -0.0956$. Rounding up, we can say we'll expect maximum log-inharmonicity shifts of one-tenth due to intonation length adjustments.

String stiffness $Q$ is harder to predict, since it presumably depends on multiple factors, like whether the string is wound and its age. For this reason, we don't obtain approximate expectations of log-inharmonicity shifts for these factors. We proceed with measurements of these various parameters to check the validity of our expectations.
