...
%\doublespacing
\begin{abstract}
We propose and evaluate an additional guitar string classification method for inharmonicity-based automatic tablature transcription using only audio. We characterize each guitar string as the linear trajectory of its log-inharmonicity with respect to its fundamental pitch, then assign unseen notes to the strings whose trajectories best approximate the notes' estimated inharmonicities. Results from classical, acoustic, and electric guitars in the RWC instruments dataset show that this method generalizes slightly better for test guitars on which the system wasn't exclusively trained than does the existing inharmonicity-based approach. We also derive and evaluate an inharmonicity compensation feature to allow for transcription of arbitrarily-tuned guitars, provided the tuning is known. Performance increases on personal recordings of alternate-tuned guitars validate this tuning adjustment feature.
...
This work builds on existing researchers' tablature transcription systems through proposal of an alternative method by which the string of an unseen note can be ascertained. We argue that our method performs slightly better than does a similar state-of-the-art system when trained on a more diverse set of reference guitars. In other words, this method's generalization is slightly better; the previous solution excels when transcribing the exact guitar on which it was trained, but ours edges ahead once the training set is made to include multiple guitars in the same class as the test guitar's.
...
The alternate-tuning experiments validated our tuning compensation feature nicely. We see in Tables~\ref{tab:resultsTune} and~\ref{tab:ag-results-tune} that the general effect of appropriately scaling the inharmonicity regressions is to restore transcription F1-scores towards that of the original best-case standard-tuned performance.
...
In conclusion, characterizing guitar strings as log-inharmonicity trajectories against pitch and classifying notes by minimizing their trajectory residuals is an effective approach to tablature transcription. Most notably, it outperforms the state-of-the-art inharmonicity-based method on most of the RWC guitar classes when trained on reference guitars that include others besides the test guitar. Furthermore, adjusting inharmonicity estimates with the compensation factor introduced in this work is effective for improving transcription accuracy on alternately-tuned guitars.
