...
%   from STFTs.
% ---------------------------------------------------
%   [B,F,T] = raspecgram(A,NFFT,Fs,window,olap, ampthresh, render_method)
%
...
%   ampthresh = cutoff amplitude e.g. -50 db
%   render_method = a string of form method_size_marker
%            where method is 'fast' or 'color' 
%            size is an integer
%            marker is 'dot' or 'circle'
%            e.g. 'fast_1_dot' means dot size 1 (NOTE dot is always size 1)
%                 'color_5_circle' means circle size 5
%
...
% -- include amplitude threshold for display
% -- Change Fmax to 0.5*Fs
% -- added fast rendering via PLOT command
% Nov 2008
%---------------------------------------------------------------------
% -- check arguments --
% use the auxiliary function, below
error( nargchk(1,7,nargin) ) %BRL4 -- change 5 to 6 to 7
[msg,A,NFFT,Fs,W,olap,ampthresh,render_method] = check_args( varargin ); %BRL4 -- add ampthresh & render_method
error( msg )
...
if nargout == 0
    ...
    Fmax = .5*Fs; %BRL4 -- was 0.3
    Fmin = 0;
    Tmax = length(A) / Fs;
    Tmin = 0;
    %ampthresh = -50; %BRL4 -- comment out and replace with input arg
    
    [X,F,T] = raframe( X, F, T, ampthresh, Fmin, Fmax, Tmin, Tmax );        
    %ratoimage( X, F, T, nhoriz, nvert, ampthresh ); %BRL4 -- add ampthresh
    
    % faster plotting method
    ratoplot( X, F, T, ampthresh, render_method ) ;
...
function [msg,x,nfft,Fs,win,nolap,ampthresh,render_method] = check_args(args)
...
%           default half the window length)
%   ampthresh = amplitude in db to surpress putput dots
%
msg = [];
...
% --- BRL4 add if stanza for threshold
...
% --- BRL4 add if stanza for render method
 if (length(args) > 6) && ~isempty(args{7})
    render_method = args{7} ;
else
